...
import io.mzlnk.javalin.ext.JavalinDIException
...
class JavalinContextTest {
    ...
    fun `should throw exception if there are defined multiple explicit lists of singletons for given type`() {
        ...
        exception.hasMessage("Multiple candidates found for java.util.List<? extends io.mzlnk.javalin.ext.internal.context.ComponentA>")
    }
    ...
    fun `should throw exception if there are multiple candidates for given component dependency`() {
        ...
        // then:
        exception.isInstanceOf(JavalinDIException::class.java)
        exception.hasMessage("Multiple candidates found for io.mzlnk.javalin.ext.internal.context.ComponentA")
    }
    ...
    fun `should throw exception if there are no candidates for given component dependency`() {
        ...
        // then:
        exception.isInstanceOf(JavalinDIException::class.java)
        exception.hasMessage("No candidates found for io.mzlnk.javalin.ext.internal.context.ComponentA")
    }
    ...
    fun `should throw exception if there is a dependency cycle between components`() {
        ...
        exception.hasMessage(
            ...
            Cycle #1:
            ┌->io.mzlnk.javalin.ext.internal.context.ComponentA -> io.mzlnk.javalin.ext.internal.context.ComponentC -> io.mzlnk.javalin.ext.internal.context.ComponentB -┐
            └------------------------------------------------------------------------------------------------------------------------------------------------------------┘
            """.trimIndent()
        )
    }
