...
internal class JavalinContext {
    ...
    fun <T : Any> findInstance(identifier: SingletonDefinition.Identifier<T>): T? {
        ...
        return matching
            .also { if (matching.size > 1) throw io.mzlnk.javalin.ext.internal.context.multipleCandidatesFoundException(
                identifier
            )
            }
            .firstOrNull()
    ...
    internal companion object {
        ...
        fun create(definitions: List<SingletonDefinition<*>>): JavalinContext {
            ...
            if (dependencyGraph.hasCycles) {
                throw io.mzlnk.javalin.ext.internal.context.dependencyCycleFoundException(dependencyGraph.cycles)
            }
                ...
                context.registerSingleton(
                    ...
                    instance = definition.instanceProvider.invoke(
                        definition.dependencies.map { dependency ->
                            context.findInstance(dependency) ?: throw io.mzlnk.javalin.ext.internal.context.noCandidatesFoundException(
                                dependency
                            )
                        }
                    )
